/**
 * 金铲铲风格 · 简化单机闯关（控制台版）
 * 编译: g++ -std=c++17 -O2 -o game golden_spatula_console.cpp
 */
...
std::vector<std::string> heroFrame(const Character& hero, int frameIdx, bool attack, bool hit) {
  std::vector<std::string> s = heroModel(hero.name);
  if (s.empty()) s = {"  .-.  ", " (o o) ", " /|_|\\ ", "  / \\  "};

  for (auto& line : s) {
    if (frameIdx == 1) line = " " + line;
    if (frameIdx == 2) line = "  " + line;
  }
  if (attack && !s.empty()) s[s.size() / 2] += "==>";
  if (hit && s.size() > 1) s[1] = " " + repeat('!', 3) + " " + s[1];
  return s;
}

void drawBattleVisual(const GameState& g, const Monster& m, int heroPos, int monPos, int frameIdx,
                      const std::string& action, bool attack, bool hit) {
  clearScreen();
  auto hero = heroFrame(g.hero, frameIdx, attack, hit);
  ...
  std::cout << "控制: [a前进 d后退 j攻击 k防御]\n";
}

char askBattleAction() {
  while (true) {
    std::cout << "输入操作: ";
    std::string line;
    if (!std::getline(std::cin, line)) return 'j';
    if (line.empty()) continue;
    char c = static_cast<char>(std::tolower(static_cast<unsigned char>(line[0])));
    if (c == 'a' || c == 'd' || c == 'j' || c == 'k') return c;
    std::cout << "无效操作，请输入 a/d/j/k。\n";
  }
}
...
CombatResult runBattle(GameState& g, Monster m) {
  ...
  while (g.hero.hp > 0 && m.hp > 0) {
    ...
    drawBattleVisual(g, m, heroPos, monPos, 0, "待机", false, false);
    char action = askBattleAction();

    if (action == 'a') {
      heroPos = std::min(heroPos + 3, monPos - 10);
      for (int f = 0; f < 3; ++f) {
        drawBattleVisual(g, m, heroPos, monPos, f, "前进", false, false);
        std::this_thread::sleep_for(std::chrono::milliseconds(60));
      }
    } else if (action == 'd') {
      heroPos = std::max(0, heroPos - 3);
      for (int f = 0; f < 3; ++f) {
        drawBattleVisual(g, m, heroPos, monPos, f, "后退", false, false);
        std::this_thread::sleep_for(std::chrono::milliseconds(60));
      }
    } else if (action == 'k') {
      for (int f = 0; f < 3; ++f) {
        drawBattleVisual(g, m, heroPos, monPos, f, "防御架势", false, false);
        std::this_thread::sleep_for(std::chrono::milliseconds(60));
      }
    }
    ...
    if (action == 'j') {
      ...
      if (dist <= 16) {
        ...
        for (int f = 0; f < 3; ++f) {
          drawBattleVisual(g, m, heroPos, monPos, f, "挥砍攻击", true, false);
    ...
    g.hero.hp -= md;
    for (int f = 0; f < 3; ++f) {
      drawBattleVisual(g, m, heroPos, monPos, f, m.name + " 反击", false, true);
      std::this_thread::sleep_for(std::chrono::milliseconds(70));
    }
